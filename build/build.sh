...
#     eg: "v1.1.1" will name the bundle "metroidprime2_apworld-v1.1.1"
...
function mk_apworld() {
  ...
  echo "From: ${root}"
  echo "To: ${destdir}"
  mkdir --parents "${destdir}/metroidprime2"
  rsync --progress \
    --recursive \
    --prune-empty-dirs \
    --exclude-from="${CWD}/apworld.ignore" \
    "${root}/" "${destdir}/metroidprime2"

  echo "${tag}" >"${destdir}/metroidprime2/version.txt"

  # If this already exists then ovewrite it
  rm -rf "${destdir}/metroidprime2/lib"
  mv "${destdir}/lib" "${destdir}/metroidprime2/lib"
  pushd "${destdir}"
  zip -9r "metroidprime2.apworld" "metroidprime2"
  popd

  rm --force --recursive "${destdir}/metroidprime2"
}
...
function main() {
  ...
  local bundle_base="metroidprime2_apworld"
